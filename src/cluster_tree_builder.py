...
from process import get_text
from process import get_node_list


logger = get_logger(__name__)

class ClusterTreeConfig(TreeBuilderConfig):
    def __init__(self,reduction_dimension=10,clustering_algorithm=RAPTOR_Clustering,clustering_params={},*args,**kwargs):
...
class ClusterTreeBuilder(TreeBuilder):
    ...
    def construct_tree(
        ...
        def process_cluster(
            ...
            node_texts = get_text(cluster) # 把group_name和group_desc汇总一下

            summarized_text = self.summarzie(
                context=node_texts,
                max_tokens=summarization_length,
            )
            logger.info("==================")
            logger.info(f"input:{node_texts}")
            logger.info(f"cluster:{summarized_text}")
            ...
            __, new_parent_node = self.create_node(
                next_node_index, group, {node.index for node in cluster}
            )
